    ...
    for b in range(0, len(carDataDF)):
        for c in range(0, len(shiftsByCar[str(b)])):
            ...
            if time == startS:
                ...
                print("Car " + str(b) + " leaving centre with " + str(batt) + "%")
...
def charge(carDataDF, carNum, chargeRate):
    ...
    if len(chargeCen) >= 1:
        ...
        if len(chargeDF) >= 1:
            ...
            for c in range(len(chargeDF)):
                d = chargeDF.index[c]
                print("car "+str(d)+" charging at "+str(chargeRate))
                carDataDF = charge(carDataDF, d, chargeRate)
    ...
    if len(chargeCen) >= 1:
        ...
        for h in range(0, len(leaveTimes)):
            ...
            if energyLeft >= 0 and batt < 100:
                chargeRate = maxRate
                print("car "+str(car)+" charging at "+str(chargeRate))
                charge(carDataDF, car, chargeRate)
